...
function isHoldingCarryObjectForJumpDown()
	...
	local ok, heldObjects = pcall(function()
		return findLocalHeldObjectsForDrop()
	end)

	if ok and heldObjects then
...
function performAirJump()
	...
	pcall(function()
		holdingCarryObject = isHoldingCarryObjectForJumpDown()
	end)

	if holdingCarryObject then
		protectCarryJumpMoment()
	else
		startToolGuard()
	end
